...
class ClangTidy(YavideService):
    ...
    def __startup_callback(self, args):
        ...
        if ext == '.json':  # In case we have a JSON compilation database we simply use one
            ...
            logging.info("Clang-Tidy compiler arguments configured with JSON database.")
            ...
            logging.info("Clang-Tidy compiler arguments configured inline with '{0}'.".format(self.compiler_options))
